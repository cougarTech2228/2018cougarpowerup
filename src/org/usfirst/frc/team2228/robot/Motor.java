...
public class Motor {
	...
	public Motor(int CanID) {
		...
		// tweak the voltage bus measurement filter, default is 32 cells in
		// rolling average (1ms per sample)
		...
		// Configure the velocity measurement period and sample window rolling
		// average
	...
	public void TeleopInit() {
		...
		Motor.selectProfileSlot(SLOT_IDx, PID_IDx);
		Motor.configAllowableClosedloopError(PID_IDx, SRXDriveBaseCfg.kClosedLoopErr, Timeout);
		Motor.config_kF(PID_IDx, FeedForwardGain, Timeout);
	...
	public void SetRamp(double milliseconds) {
		if (CloseLoopEnabled)
			...
			Motor.configOpenloopRamp(milliseconds, Timeout);
	}
	...
	public boolean AutoMove(int encoders, double speed) {
		if(Math.abs(encoders) <= Math.abs(Motor.getSensorCollection().getQuadraturePosition())) {
			Motor.set(0);
			System.out.println("Finished with " + Motor.getSensorCollection().getQuadraturePosition() + " encoder counts");
			return true;
		}
		else {
			Motor.set(speed);
