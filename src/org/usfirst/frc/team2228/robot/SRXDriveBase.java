...
public class SRXDriveBase {
	...
	private int mmRightCruiseVel = 0;
	private int mmRightAccel = 0;
	private int mmLeftCruiseVel = 0;
	private int mmLeftAccel = 0;
	private double mmRightDistance = 0;
	private double mmLeftDistance = 0;
	...
	*/
	...
	public boolean SRXCalStdTrapezoidMoveMagic(double _SRXRightDistanceIn, double _SRXRightTimeSec, double _SRXLeftDistanceIn, double _SRXLeftTimeSec) {
		...
		if(!isSRXCalStdTrapezoidMoveMagicActive){
			...
			mmLeftDistance = (int)(_SRXLeftDistanceIn / SRXDriveBaseCfg.kInchesPerCount);
			mmLeftCruiseVel = (int)(1.5 * (mmLeftDistance/(_SRXLeftTimeSec / SRXDriveBaseCfg.kvelMeasurePeriodSec)));
			mmLeftAccel = (int)((4.5 * mmLeftDistance) / ((_SRXLeftTimeSec / SRXDriveBaseCfg.kvelMeasurePeriodSec) * (_SRXLeftTimeSec / SRXDriveBaseCfg.kvelMeasurePeriodSec)));
			
			mmRightDistance = (int)( _SRXRightDistanceIn / SRXDriveBaseCfg.kInchesPerCount);
			mmRightCruiseVel = (int)(1.5 * (mmRightDistance/(_SRXRightTimeSec / SRXDriveBaseCfg.kvelMeasurePeriodSec)));
			mmRightAccel = (int)((4.5 * mmRightDistance) / ((_SRXRightTimeSec / SRXDriveBaseCfg.kvelMeasurePeriodSec) * (_SRXRightTimeSec / SRXDriveBaseCfg.kvelMeasurePeriodSec)));
		...
		} else if(!magicMove(mmRightCruiseVel, mmRightAccel, mmRightDistance, mmLeftCruiseVel,	mmLeftAccel, mmLeftDistance)){
			isSRXCalStdTrapezoidMoveMagicActive = false;
			methodTime = Timer.getFPGATimestamp() - methodStartTime;
	...
	public boolean SRXMoveMagic(double _SRXMoveDistanceIn, double _SRXMoveTimeSec) {
		if(!isSRXMoveMagicActive){
			msg("START MOTION MAGIC CALCULATIONS ==================================");
			methodStartTime = Timer.getFPGATimestamp();
			isSRXMoveMagicActive = true;
			mmLeftDistance = (int)(_SRXMoveDistanceIn / SRXDriveBaseCfg.kInchesPerCount);
			mmLeftCruiseVel = (int)(15 * (mmLeftDistance/(_SRXMoveTimeSec / SRXDriveBaseCfg.kvelMeasurePeriodSec)));			
			mmLeftAccel = (int)((45 * mmLeftDistance) / ((_SRXMoveTimeSec / SRXDriveBaseCfg.kvelMeasurePeriodSec) * (_SRXMoveTimeSec / SRXDriveBaseCfg.kvelMeasurePeriodSec)));
	...
	public boolean SRXRotateMagic(double _SRXRotateAngleDeg, double _SRXRotateTimeSec) {
	...
	public boolean magicMove(int _rightCruiseVel, int _rightAccel, double _rightDistance, int _leftCruiseVel,
		...
		if (!isSRXMagicMoveActive) {
			...
			if ((Math.abs(motionMagicLeftPos) >= Math.abs(_leftDistance)) && (Math.abs(motionMagicRightPos) >= Math.abs(_rightDistance))) {
				driveRightMasterMtr.set(ControlMode.MotionMagic, 0); 
				driveLeftMasterMtr.set(ControlMode.MotionMagic, 0);
				isSRXMagicMoveActive = false;
				methodTime = Timer.getFPGATimestamp() - methodStartTime;
