...
//			// Adds movement to the auto sequence
			Cg.addSequential(
					new MoveTo(base, (Dimensions.AUTOLINE_TO_ALLIANCE - Dimensions.LENGTH_OF_ROBOT), speed, false ,false, 3.0));
		...
		case "Left Switch":
			...
			if (data == GameData.firstIndexL) {
				...
				Cg.addParallel(new MoveTo(base, 19, speed / 2, false, false, 1.0), 3);
...
//				Cg.addParallel(new MoveTo(base, 3, speed / 2, false, 2.0));
		...
		case "Right Switch Turn":
			...
			if (data == GameData.firstIndexR) {
				...
				Cg.addParallel(new ElevatorAuto(elevator, 2.0));
				Cg.addParallel(new MoveTo(base, 19, speed / 2, false, false, 1.0));
